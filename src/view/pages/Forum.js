...
const Forum = (props) => {
  ...
  const handleLikeButton = async () => {
    api.timeline('updatelikes', { key: article?.url }, { credentials: 'include' })
    .then(() => setArticle({...article, likes: article.likes+1}))
    .then(() => console.log('SIM PORRA'))
  ...
  useEffect(() => { 
    ...
    if (type === 'art') {
      api.media('get', { filename: article?.imagem })
      .then(blob => setArt(URL.createObjectURL(blob)));
    }


  }, [article, location, scrollToTop]);
  return (
    <>
          ...
          <div className="forum-header">
          {
            ...
            <div className="d-flex flex-row h-100 gap-2">
              ...
              <div className="d-inline-flex flex-column gap-1">
                <button className="d-flex justify-content-center align-items-center rate-btn btn-success"
